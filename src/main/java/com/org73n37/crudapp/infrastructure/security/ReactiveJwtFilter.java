...
public class ReactiveJwtFilter implements WebFilter {
    ...
    // ^ Static verification key intended ONLY for local development and tests.
    //   It bypasses Keycloak JWKS retrieval, so it is refused when
    //   app.mode=PRODUCTION (see getVerificationKey) to prevent a signing-key
    //   backdoor from ever being honored in a production deployment.
    ...
    private PublicKey getVerificationKey(String token) throws Exception {
        if (testPublicKeyPem != null && !testPublicKeyPem.trim().isEmpty()) {
            // Deny-by-default in production: never honor a statically configured
            // verification key when running in PRODUCTION mode. Fall through to
            // the authoritative Keycloak JWKS endpoint instead.
            if (appModeConfig != null && appModeConfig.isProduction()) {
                log.error("[SECURITY EVENT] Action=CONFIG_REJECTED Reason=keycloak.test.public-key is set while app.mode=PRODUCTION; ignoring static test key and verifying via Keycloak JWKS");
            } else {
